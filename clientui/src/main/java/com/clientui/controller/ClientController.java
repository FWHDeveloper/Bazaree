...
public class ClientController {
    ...
    log.info ("Enovoi requete COMMANDE");
    ...
    public String payerCommande(@PathVariable int idCommande, @PathVariable Double montantCommande, Model model){
        ...
        model.addAttribute("paiementOk", paiementAccepte); // on envoi un Bool
        // ean paiementOk à la vue
    ...
    // Expeditions
    // Page d'acueil pour accéder aux liens pour verfier l'état de chaque commande par son numéro
    @RequestMapping("/")
    public String Home(Model model){

        log.info("Envoi requête vers microservice-expeditions");

        List<ExpeditionBean> expeditions =  expeditionsProxy.listeDesExpeditions();

        model.addAttribute("expeditions", expeditions);


        return "HomeExpeditions";
    }
// Page vous montre l'état de la commande

    @RequestMapping("/suivi/{id}")

    public String ficheEtat(@PathVariable int id,  Model model){

        ExpeditionBean expedition = expeditionsProxy.etatExpedition(id);
